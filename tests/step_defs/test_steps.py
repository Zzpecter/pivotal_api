...
from main.core.utils.string_utils import StringUtils as string_utils
from main.core.request_controller import RequestController
from main.pivotal.utils.api_constants import HttpMethods
...
def step_send_request(http_method, endpoint, request):
    ...
    tags_to_replace = string_utils.search_text_between_tags(endpoint)

    LOGGER.debug(f'Tags found:  {tags_to_replace}')
    LOGGER.debug(f'un-processed endpoint:  {endpoint}')

    for tag in tags_to_replace:
        endpoint = string_utils.replace_string(
            base_string=endpoint,
            replace_string=str(request.config.cache.get(tag, None)),
            pattern=f'<{tag}>')
    ...
    request.config.cache.set(
        'response',
        response.json() if http_method != HttpMethods.DELETE.value else None)
